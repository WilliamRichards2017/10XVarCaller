...
void coverage(int &size, vector<pair<int, int> > &reads) {
  ...
  float coverage = float(size)/ float(count);
...
//    barCodeCSV << hashIt->first << ","  << hashIt->second.first.second;
//    for (vecIt = hashIt->second.second.begin(); vecIt != hashIt->second.second.end(); vecIt++) {
//      barCodeCSV << ","  << vecIt[i].first;
//    }
//    barCodeCSV << "\n";
//  }
//}
...
void distanceStats(clusterList_t  &clusterList) {                                                                                                                                                 
   ofstream distance;                                                                                                                                                                              
   ...
   for (vecIt = clusterList.begin(); vecIt != clusterList.end(); vecIt++) {
	 ...
	 for (readIt = clustIt->begin(); readIt!=clustIt->end()-1; readIt++) {
	   int dist = (readIt+1)->first - readIt->first;
	   if (dist != 0) {
	   distance  << dist << endl; 
	   //  cout << dist  << endl;
	   }
